...
describe('Synthesis Engine', () => {
  ...
  it('Test 2 - SHADOW_RULE awsJsonPatch has correct structure', () => {
    ...
    // Check full Security Group structure
    expect(patch).toHaveProperty('GroupId', 'sg-synthesized-fix');
    expect(patch).toHaveProperty('GroupName', 'synthesized-security-group');
    expect(patch).toHaveProperty('Description', 'Auto-generated by CDPI to resolve conflicts');
    expect(patch).toHaveProperty('IpPermissions');
    expect(patch.IpPermissions).toHaveLength(1);
    
    // Check IpPermission structure
    const ipPermission = patch.IpPermissions[0];
    expect(ipPermission).toHaveProperty('IpProtocol', 'tcp');
    expect(ipPermission).toHaveProperty('FromPort', 443);
    expect(ipPermission).toHaveProperty('ToPort', 443);
    expect(ipPermission.IpRanges).toHaveLength(1);
    expect(ipPermission.IpRanges[0].CidrIp).toBe('10.0.1.0/24');
  });
  ...
  it('Test 3 - SHADOW_RULE with protocol=all maps to IpProtocol=tcp in patch', () => {
    ...
    expect(patch.IpPermissions[0].IpProtocol).toBe('tcp');
  });
  ...
  it('Test 7 - PERMIT_MISMATCH awsJsonPatch has full Security Group structure', () => {
    const conflict = makeConflict({
    ...
    // Check IpPermission has correct port range
    const ipPermission = patch.IpPermissions[0];
    expect(ipPermission.FromPort).toBe(443); // From Cisco rule
    expect(ipPermission.ToPort).toBe(443);
  });
